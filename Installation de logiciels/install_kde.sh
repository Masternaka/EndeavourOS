...
###############################################################################
# Script d'installation de KDE pour Arch/EndeavourOS
#
# Utilisation:
# 1. Rendez-le exécutable: chmod +x install_kde.sh
# 2. Exécutez-le: sudo ./install_kde.sh [--dry-run]
#
# Ce script installe les paquets KDE Plasma et ses extensions.
...
confirm_installation() {
  if [ "$DRY_RUN" = false ]; then
    echo -e "${YELLOW}Continuer avec l'installation de KDE ? (y/N)${RESET}"
...
show_help() {
  echo -e "${GREEN}=== Aide du script d'installation KDE ===${RESET}"
  echo ""
  echo "Ce script permet d'installer KDE Plasma et ses extensions sur Arch/EndeavourOS."
  echo ""
  echo "UTILISATION:"
  echo "  sudo ./install_kde.sh [options]"
  echo ""
  echo "OPTIONS:"
  echo "  --help      Affiche cette aide"
  echo "  --dry-run   Simule sans installer"
  echo ""
  echo "PAQUETS KDE INSTALLÉS:"
  echo "  - Plasma: kio-extras, merkuro, dolphin-plugins, kio-admin, filelight"
  echo "  - Terminal: yakuake, krusader, kwrite"
  echo "  - Système: systemdgenie, xdg-desktop-portal-kde, print-manager"
  echo "  - Multimédia: ffmpegthumbs, qt6-multimedia"
  echo "  - Addons: kdeplasma-addons, plasma-systemmonitor, kwalletmanager"
}

# Fonction installation de paquets KDE
...
show_installation_summary() {
  echo -e "${GREEN}=== Résumé de l'installation KDE ===${RESET}"
  echo -e "Date: $(date)"
  echo -e "Paquets installés: $(pacman -Q | wc -l)"
  echo -e "${GREEN}Installation KDE terminée avec succès !${RESET}"
}
...
echo -e "${GREEN}=== Configuration KDE ===${RESET}"
